...
contract HubbleReferral is Governable {
    ...
    function _createReferralCode(
        ...
        require(
            bytes(_referralCode).length >= 4,
            "HR: referral code too short"
        );

        address existingReferrer = referralCodeToReferrerMap[_referralCode];
        require(
            existingReferrer == address(0x0),
            "HR: referral code already exists"
        );

        require(
            bytes(referrers[_referrer].referralCode).length == 0,
    ...
    function _setReferralCode(string calldata _referralCode, address _trader)
        internal
    {
        require(
            bytes(_referralCode).length >= 4,
        ...
        require(
            bytes(traders[_trader].referralCode).length == 0,
    ...
    function getReferralCodeByAddress(address _referrer)
        external
        view
    ...
    function getTraderRefereeInfo(address _trader)
        external
        view
