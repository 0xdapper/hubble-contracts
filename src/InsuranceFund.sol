...
contract InsuranceFund is VanillaGovernable, ERC20Upgradeable {
    ...
    function deposit(uint _amount) external {
        ...
        if (_totalSupply == 0 && vusdBalance > 0) {
            // trading fee accumulated while there were no IF LPs
    ...
    function unbondShares(uint shares) external {
        ...
        uint unbondTime = ((_now + unbondPeriod) / unbondRoundOff) *
            unbondRoundOff;
    ...
    function withdraw(uint shares) external {
        ...
        require(
            !_hasWithdrawPeriodElapsed(_now, unbond[usr].unbondTime),
    ...
    function startAuction(address token) external onlyMarginAccount {
        if (
            !_isAuctionOngoing(
                auctions[token].startedAt,
                auctions[token].expiryTime
            )
        ) {
    ...
    function buyCollateralFromAuction(address token, uint amount) external {
        ...
        // validate auction
        require(
            _isAuctionOngoing(auction.startedAt, auction.expiryTime),
    ...
    function _beforeTokenTransfer(
        ...
        if (
            !_hasWithdrawPeriodElapsed(
                _blockTimestamp(),
                unbond[from].unbondTime
            )
        ) {
            require(
                amount <= balanceOf(from) - unbond[from].shares,
                "shares_are_unbonding"
            );
        }
    }

    function _hasWithdrawPeriodElapsed(uint _now, uint _unbondTime)
        internal
        view
        returns (bool)
    {
    ...
    function _getAuctionPrice(Auction memory auction)
        internal
        view
        returns (uint)
    {
        uint diff = (auction.startPrice *
            (_blockTimestamp() - auction.startedAt)) / auctionDuration;
        return auction.startPrice - diff;
    }

    function _isAuctionOngoing(uint startedAt, uint expiryTime)
        internal
        view
        returns (bool)
    {
    ...
    function _calcVusdAmountForAuction(
    ...
    function _totalPoolValue() internal view returns (uint totalBalance) {
        IMarginAccount.Collateral[] memory assets = IMarginAccount(
            marginAccount
        ).supportedAssets();

        for (uint i; i < assets.length; i++) {
            uint _balance = IERC20(address(assets[i].token)).balanceOf(
                address(this)
            );
            if (_balance == 0) continue;

            uint numerator = _balance *
                uint(oracle.getUnderlyingPrice(address(assets[i].token)));
