...
import {ERC20Detailed, IClearingHouse, IInsuranceFund, IOracle, IRegistry, IMarginAccount, IERC20FlexibleSupply, IWAVAX} from "./Interfaces.sol";
...
contract MarginAccount is IMarginAccount, HubbleBase, ReentrancyGuard {
     ...
     */
    mapping(uint => mapping(address => int)) public override margin;
    ...
    function initialize(address _governance, address _vusd)
        external
        initializer
    {
     ...
     */
    function addMargin(uint idx, uint amount) external override whenNotPaused {
    ...
    function addMarginFor(
        ...
        if (idx == VUSD_IDX) {
            ...
            supportedCollateral[idx].token.safeTransferFrom(
                _msgSender(),
                address(this),
                amount
            );
     ...
     */
    function removeMargin(uint idx, uint256 amount)
        external
        override
        whenNotPaused
    {
    ...
    function isLiquidatable(address trader, bool includeFunding)
        ...
        override
        returns (
            IMarginAccount.LiquidationStatus _isLiquidatable,
            uint repayAmount,
            uint incentivePerDollar
        )
    {
        ...
        (uint256 notionalPosition, ) = clearingHouse
            .getTotalNotionalPositionAndUnrealizedPnl(
                trader,
                0,
                IClearingHouse.Mode.Min_Allowable_Margin
            ); // last two arguments are irrelevent as we are checking only for zero/non-zero notional position in next step
        if (notionalPosition != 0) {
            // Liquidate positions before liquidating margin account
    ...
    function liquidateFlexible(
        ...
        for (uint i; i < idxs.length; i++) {
            LiquidationBuffer memory buffer = _getLiquidationInfo(
                trader,
                idxs[i]
            );
            // revert only if trader has open positions, otherwise fail silently
            if (
                buffer.status == IMarginAccount.LiquidationStatus.OPEN_POSITIONS
            ) {
                revert NOT_LIQUIDATABLE(buffer.status);
            }
            if (
                buffer.status !=
                IMarginAccount.LiquidationStatus.IS_LIQUIDATABLE
            ) {
                break;
            }
    ...
    function settleBadDebt(address trader) external whenNotPaused {
        ...
        require(
            notionalPosition == 0,
            "Liquidate positions before settling bad debt"
        );
     ...
     */
    function _getLiquidationInfo(address trader, uint idx)
        internal
        view
        returns (LiquidationBuffer memory buffer)
    {
        require(
            idx > VUSD_IDX && idx < supportedCollateral.length,
            "collateral not seizable"
        );
        (
            buffer.status,
            buffer.repayAble,
            buffer.incentivePerDollar
        ) = isLiquidatable(trader, false);
        if (buffer.status == IMarginAccount.LiquidationStatus.IS_LIQUIDATABLE) {
            Collateral memory coll = supportedCollateral[idx];
            buffer.priceCollateral = oracle
                .getUnderlyingPrice(address(coll.token))
                .toUint256();
    ...
    function _executeLiquidation(
        ...
        internal
        returns (
            uint /* left over repayable */
        )
    {
        if (repay == 0 || seize == 0) {
            // provides more flexibility, so prefer not reverting
            return repayAble;
        }
    ...
    function _seizeToRepay(LiquidationBuffer memory buffer, uint seize)
        internal
        pure
        returns (uint repay)
    {
    ...
    function getSpotCollateralValue(address trader)
        public
        view
        override
        returns (int256 spot)
    ...
    function getNormalizedMargin(address trader)
        public
        view
        override
        returns (int256 weighted)
    {
    ...
    function supportedAssetsLen() external view override returns (uint) {
    ...
    function _addCollateral(address _coin, uint _weight) internal {
        ...
        for (uint i; i < _collaterals.length; i++) {
            require(
                address(_collaterals[i].token) != _coin,
                "collateral exists"
            );
        }
    ...
    function _validateRemoveMargin(
        uint idx,
        uint256 amount,
        address trader
    ) internal {
        ...
        require(
            margin[VUSD_IDX][trader] >= 0,
            "Cannot remove margin when vusd balance is negative"
        );
        require(
    ...
    function syncDeps(address _registry, uint _liquidationIncentive)
        public
        onlyGovernance
    {
        // protecting against setting a very high liquidation incentive. Max 10%
        require(
            _liquidationIncentive <= PRECISION / 10,
            "MA.syncDeps.LI_GT_10_percent"
        );
    ...
    function whitelistCollateral(address _coin, uint _weight)
        external
        onlyGovernance
    {
    ...
    function changeCollateralWeight(uint idx, uint _weight)
        external
        onlyGovernance
    {
        require(_weight <= PRECISION, "weight > 1e6");
