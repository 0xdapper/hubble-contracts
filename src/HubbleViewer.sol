...
contract HubbleViewer is IHubbleViewer {
    ...
    function marginAccountLiquidatationStatus(address[] calldata traders)
        ...
        view
        returns (
            IMarginAccount.LiquidationStatus[] memory isLiquidatable,
            uint[] memory repayAmount,
            uint[] memory incentivePerDollar
        )
    {
        ...
        for (uint i; i < traders.length; i++) {
            (
                isLiquidatable[i],
                repayAmount[i],
                incentivePerDollar[i]
            ) = marginAccount.isLiquidatable(traders[i], true);
        }
    }
     ...
     */
    function userPositions(address trader)
        external
        view
        returns (Position[] memory positions)
    {
        ...
        for (uint i; i < l; i++) {
            ...
            (positions[i].size, positions[i].openNotional, , ) = amm.positions(
                trader
            );
            if (positions[i].size == 0) {
                ...
                (, positions[i].unrealizedPnl) = amm
                    .getTakerNotionalPositionAndUnrealizedPnl(trader);
                positions[i].avgOpen =
                    (positions[i].openNotional * 1e18) /
                    _abs(positions[i].size).toUint256();
            }
        }
    }
     ...
     *   positions[i].avgOpen - Average $ value at which position was started
     */
    function makerPositions(address maker)
        external
        view
        returns (Position[] memory positions)
    {
    ...
    function getTakerExpectedMFAndLiquidationPrice(
        ...
        view
        returns (
            int256 expectedMarginFraction,
            uint256 quoteAssetQuantity,
        ...
        )
    {
        ...
        (uint256 notionalPosition, int256 margin) = clearingHouse
        ...
        returns (int256 expectedMarginFraction, uint256 liquidationPrice)
    {
...

        {
            ...
            if (isRemove) {
                ...
                maker.vUSD =
                    (maker.vUSD * (maker.dToken - dToken)) /
                    maker.dToken;
            } else {
                maker.vUSD += vUSD;
            }
...

        {
            ...
            (int256 makerPosSize, , ) = getMakerPositionAndUnrealizedPnl(
                trader,
                idx
            );
            uint totalPosNotional = amm.getCloseQuote(
                makerPosSize + takerPosSize
            );
            notionalPosition += _max(
                2 * maker.vUSD + takerNotional,
                totalPosNotional
            );
        }

        {
            (uint nowNotional, , , ) = amm.getNotionalPositionAndUnrealizedPnl(
                trader
            );
    ...
    function getLiquidationPrice(address trader, uint idx)
        external
        view
        returns (uint liquidationPrice)
    {
        ...
        (uint256 notionalPosition, int256 margin) = clearingHouse
            .getNotionalPositionAndMargin(
                trader,
                true, /* includeFundingPayments */
                IClearingHouse.Mode.Maintenance_Margin
            );
        IAMM amm = clearingHouse.amms(idx);
        liquidationPrice = _getLiquidationPrice(
            trader,
    ...
    function _getLiquidationPrice(
        ...
        if (baseAssetQuantity != 0) {
            ...
            if (baseAssetQuantity * totalPosSize >= 0) {
                // increasingPosition i.e. same direction trade
                ...
                if (_abs(totalPosSize) >= _abs(baseAssetQuantity)) {
                    // position side remains same after the trade
                    ...
                    // position side changes after the trade
        ...
        int256 pnlForLiquidation = (clearingHouse.maintenanceMargin() *
        ...
        if (totalPosSize > 0) {
            _liquidationPrice =
                ((openNotional.toInt256() + pnlForLiquidation) * 1e18) /
                totalPosSize;
    ...
    {
        return
            _getMakerPositionAndUnrealizedPnl(_maker, clearingHouse.amms(idx));
    }
     ...
     * @param idx amm index
     */
    function getQuote(int256 baseAssetQuantity, uint idx)
        public
        view
     ...
     */
    function calcWithdrawAmounts(uint dToken, uint idx)
        external
        view
        returns (uint quoteAsset, uint baseAsset)
    {
        IAMM amm = clearingHouse.amms(idx);
        IVAMM vamm = amm.vamm();
    ...
    function getMakerQuote(
        ...
        bool isBase,
        bool deposit
    ) public view returns (uint fillAmount, uint dToken) {
        IAMM amm = clearingHouse.amms(idx);
        IVAMM vamm = amm.vamm();
    ...
    function userInfo(address trader) external view returns (int256[] memory) {
        ...
        _margin[VUSD_IDX] =
            marginAccount.margin(VUSD_IDX, trader) -
    ...
    {
        int256 margin;
        (margin, totalCollateral) = marginAccount.weightedAndSpotCollateral(
            trader
        );
        marginFraction = clearingHouse.calcMarginFraction(
            trader,
            true,
            IClearingHouse.Mode.Min_Allowable_Margin
        );
        ...
        for (uint i; i < l; i++) {
            ...
            IAMM.Maker memory maker = amm.makers(trader);
            if (
                amm.isOverSpreadLimit() &&
                (size != 0 || maker.dToken != 0 || maker.ignition != 0)
            ) {
        ...
        if (isOverSpreadLimit) {
            marginFractionLiquidation = clearingHouse.calcMarginFraction(
                trader,
                true,
                IClearingHouse.Mode.Maintenance_Margin
            );
        ...
        (notionalPosition, unrealizedPnl) = clearingHouse
        ...
        freeMargin =
            ...
            unrealizedPnl -
            clearingHouse.getTotalFunding(trader) -
            (notionalPosition.toInt256() * minAllowableMargin) /
            PRECISION_INT;
    }
        ...
        returns (int[][] memory takerFundings, int[][] memory makerFundings)
    {
        ...
        for (uint j; j < t; j++) {
            ...
            for (uint i; i < l; i++) {
                IAMM amm = clearingHouse.amms(i);
                (takerFundings[j][i], makerFundings[j][i], , ) = amm
                    .getPendingFundingPayment(traders[j]);
            }
        }
    }
