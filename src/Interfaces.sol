...
interface IOracle {
    ...
    function getUnderlyingTwapPrice(address asset, uint256 intervalInSeconds)
        external
        view
        returns (int256);
}
...
interface IClearingHouse {
    ...
    function getTotalNotionalPositionAndUnrealizedPnl(
        ...
        int256 margin,
        Mode mode
    ) external view returns (uint256 notionalPosition, int256 unrealizedPnl);
    ...
    function getNotionalPositionAndMargin(
        ...
        Mode mode
    ) external view returns (uint256 notionalPosition, int256 margin);
...
interface IInsuranceFund {
    ...
    function calcVusdAmountForAuction(address token, uint amount)
        external
        view
        returns (uint);
...
interface IAMM {
    ...
    function positions(address trader)
        external
        view
        returns (
            int256,
    ...
    function getOpenNotionalWhileReducingPosition(
        ...
        int256 unrealizedPnl,
        int256 baseAssetQuantity
    ) external pure returns (uint256 remainOpenNotional, int realizedPnl);
    ...
    ) external view returns (uint, int256);
}
...
interface IHubbleReferral {
    function getTraderRefereeInfo(address trader)
        external
        view
...
interface IJoeRouter02 {
    ...
    function getAmountsOut(uint256 amountOut, address[] calldata path)
        external
        returns (uint256[] memory amounts);
}
