...
contract AMM is IAMM, Governable {
    ...
    Ignition public override ignition;
    IAMM.AMMState public override ammState;
    ...
    event FundingRateUpdated(
        ...
        int256 cumulativePremiumPerDtoken,
        ...
        )
    {
        ...
        if (
            isNewPosition ||
            (position.size > 0 ? Side.LONG : Side.SHORT) == side
        ) {
            // realizedPnl = 0;
            quoteAsset = _increasePosition(
                trader,
                baseAssetQuantity,
                quoteAssetLimit
            );
        ...
        require(
            totalPosSize == 0 || totalPosSize >= minSizeRequirement,
            "position_less_than_minSize"
        );
        ...
        )
    {
        ...
        if (toLiquidate != pozSize && ((toLiquidate * 101) / 100) >= pozSize) {
        ...
        while (
            reserveSnapshots[index].blockNumber == block.number && index != 0
        ) {
        ...
        require(
            diff <= (lastTradePrice * maxLiquidationPriceSpread) / 1e8,
            "AMM.liquidation_price_slippage"
        );

        // liquidate position
        if (isLongPosition) {
            baseAsset = -toLiquidate.toInt256();
            (realizedPnl, quoteAsset) = _reducePosition(
                trader,
                baseAsset,
                0,
                true /* isLiquidation */
    ...
    function addLiquidity(
        address maker,
        uint baseAssetQuantity,
        uint minDToken
    ) external override onlyClearingHouse whenActive returns (uint dToken) {
        ...
        if (_maker.dToken > 0) {
            // Maker only accumulates position when they had non-zero liquidity
            _maker.pos +=
                ((posAccumulator - _maker.posAccumulator) *
                    _maker.dToken.toInt256()) /
                1e18;
        }
    ...
    function unbondLiquidity(uint dToken) external whenActive {
        ...
        _maker.unbondTime =
            ((_blockTimestamp() + unbondPeriod) / unbondRoundOff) *
            unbondRoundOff;
        ...
        )
    {
        Maker storage _maker = _makers[maker];
        require(
            _maker.unbondAmount >= amount,
            "withdrawing_more_than_unbonded"
        );
        unchecked {
            _maker.unbondAmount -= amount;
        }
        ...
        require(
            _now <= _maker.unbondTime + withdrawPeriod,
            "withdraw_period_over"
        );
        ...
        (realizedPnl, makerOpenNotional, makerPosition) = _removeLiquidity(
            maker,
            amount,
            varGroup1
        );
        if (_maker.dToken != 0) {
            ...
            require(
                _maker.vAsset >= minSizeRequirement,
                "leftover_liquidity_is_too_less"
            );
            uint totalPosSize = uint(abs(positions[maker].size));
            require(
                totalPosSize == 0 || totalPosSize >= minSizeRequirement,
                "removing_very_small_liquidity"
            );
        }
    }
        ...
        )
    {
        ...
        // translate impermanent position to a permanent one
        {
            if (makerPosition != 0) {
                ...
                if (makerPosition * position.size < 0) {
                    // this ensures takerPosition !=0
                    realizedPnl += _getPnlWhileReducingPosition(
                        position.size,
                        position.openNotional,
                        makerPosition
                    );
    ...
    {
        require(
            abs(positionSize) >= abs(baseAssetQuantity),
            "AMM.ONLY_REDUCE_POS"
        );
        bool isLongPosition = positionSize > 0 ? true : false;

        realizedPnl =
            (unrealizedPnl * abs(baseAssetQuantity)) /
            abs(positionSize);
        ...
        if (isLongPosition) {
             ...
             */
            remainOpenNotional = (newNotionalPosition.toInt256() -
                unrealizedPnlAfter).toUint256(); // will assert that remainOpenNotional >= 0
        } else {
            /**
             * Let baseAssetQuantity = Q, position.size = size, by definition of _reducePosition, abs(size) >= abs(Q)
             * quoteAsset = notionalPosition * Q / size
     ...
     */
    function settleFunding() external override onlyClearingHouse {
        if (ammState != AMMState.Active || _blockTimestamp() < nextFundingTime)
            return;
    ...
    function liftOff() external onlyGovernance whenIgnition {
        uint256 underlyingPrice = getUnderlyingTwapPrice(15 minutes)
            .toUint256();
        ...
        if (ignition.quoteAsset > 0) {
            ...
            ignition.dToken = vamm.add_liquidity(
                [ignition.quoteAsset, ignition.baseAsset],
                0
            );
    ...
    function getIgnitionShare(uint vUSD)
        public
        view
        override
        returns (uint vAsset, uint dToken)
    {
    ...
    {
        return
            oracle.getUnderlyingTwapPrice(underlyingAsset, _intervalInSeconds);
    }
        ...
        )
    {
        ...
        (notionalPosition, size, unrealizedPnl, openNotional) = vamm
            .get_notional(
                dToken,
                vUSD,
                vAsset,
     ...
     */
    function getOracleBasedPnl(
        address trader,
        int256 margin,
        IClearingHouse.Mode mode
    )
        external
        view
        override
        returns (uint notionalPosition, int256 unrealizedPnl)
    {
        ...
        int256 marginFraction = ((margin + unrealizedPnl) * 1e6) /
            notionalPosition.toInt256();
        (
            int oracleBasedNotional,
            int256 oracleBasedUnrealizedPnl,
            int256 oracleBasedMF
        ) = _getOracleBasedMarginFraction(trader, margin, openNotional, size);
        ...
        } else if (oracleBasedMF < marginFraction) {
    ...
    function _getOracleBasedMarginFraction(
        address trader,
        int256 margin,
        uint256 openNotional,
        int256 size
        ...
        )
    {
        ...
        if (size > 0) {
            oracleBasedUnrealizedPnl =
                oracleBasedNotional -
                openNotional.toInt256();
        } else if (size < 0) {
            oracleBasedUnrealizedPnl =
                openNotional.toInt256() -
                oracleBasedNotional;
        ...
        int256 oracleBasedTakerNotional = (oraclePrice *
            abs(positions[trader].size)) / BASE_PRECISION;
        oracleBasedNotional = _max(
            2 * _makers[trader].vUSD.toInt256() + oracleBasedTakerNotional,
            oracleBasedNotional
        );
        marginFraction =
            ((margin + oracleBasedUnrealizedPnl) * 1e6) /
            oracleBasedNotional;
    }
        ...
        )
    {
        ...
        // Taker
        takerFundingPayment =
            ((latestCumulativePremiumFraction - taker.lastPremiumFraction) *
                taker.size) /
            BASE_PRECISION;
        ...
        if (dToken > 0) {
            int256 cpf = latestCumulativePremiumFraction -
                maker.lastPremiumFraction;
            makerFundingPayment =
                (maker.pos *
                    cpf +
    ...
    function getCloseQuote(int256 baseAssetQuantity)
        public
        view
        override
        returns (uint256 quoteAssetQuantity)
    {
    ...
    function getTakerNotionalPositionAndUnrealizedPnl(address trader)
        public
        view
        override
        returns (uint takerNotionalPosition, int256 unrealizedPnl)
    {
        Position memory position = positions[trader];
        if (position.size > 0) {
            takerNotionalPosition = vamm.get_dy(
                1,
                0,
                position.size.toUint256()
            );
            unrealizedPnl =
                takerNotionalPosition.toInt256() -
                position.openNotional.toInt256();
        } else if (position.size < 0) {
            takerNotionalPosition = vamm.get_dx(
                0,
                1,
                (-position.size).toUint256()
            );
    ...
    function makers(address maker)
        external
        view
        override
        returns (Maker memory)
    {
    ...
    */
    function _long(
        ...
        uint max_dx,
        bool isLiquidation
    ) internal returns (uint256 quoteAssetQuantity) {
        ...
        posAccumulator -=
            (baseAssetQuantity * 1e18) /
            vamm.totalSupply().toInt256();
    ...
    */
    function _short(
        ...
        require(
            baseAssetQuantity < 0,
            "VAMM._short: baseAssetQuantity is >= 0"
        );
    ...
    function _getPnlWhileReducingPosition(
        ...
        if (abs(takerPosition) > abs(makerPosition)) {
            // taker position side remains same
            uint reducedOpenNotional = (takerOpenNotional *
                abs(makerPosition).toUint256()) /
                abs(takerPosition).toUint256();
            uint makerNotional = (newNotional *
                abs(makerPosition).toUint256()) / totalPosition;
            ...
            // taker position side changes
            // @todo handle case when totalPosition = 0
            uint closedPositionNotional = (newNotional *
                abs(takerPosition).toUint256()) / totalPosition;
            pnlToBeRealized = _getPnlToBeRealized(
                takerPosition,
    ...
    function _increasePosition(
        ...
        int256 baseAssetQuantity,
        uint quoteAssetLimit
    ) internal returns (uint quoteAsset) {
        if (baseAssetQuantity > 0) {
            // Long - purchase baseAssetQuantity
        ...
        )
    {
        ...
        if (abs(position.size) >= abs(baseAssetQuantity)) {
            ...
            uint closedRatio = (quoteAssetLimit *
                abs(position.size).toUint256()) /
                abs(baseAssetQuantity).toUint256();
            (realizedPnl, quoteAsset) = _reducePosition(
                trader,
                -position.size,
                closedRatio,
            ...
            quoteAsset += _increasePosition(
                trader,
                baseAssetQuantity + position.size,
                quoteAssetLimit
            );
            isPositionIncreased = true;
        }
    ...
    function _reducePosition(
        ...
        uint quoteAssetLimit,
        bool isLiquidation
    ) internal returns (int realizedPnl, uint256 quoteAsset) {
        (, int256 unrealizedPnl) = getTakerNotionalPositionAndUnrealizedPnl(
            trader
        );
        ...
        uint256 notionalPosition = getCloseQuote(
            position.size + baseAssetQuantity
        );
        (
            position.openNotional,
            realizedPnl
        ) = getOpenNotionalWhileReducingPosition(
            position.size,
            notionalPosition,
            unrealizedPnl,
            baseAssetQuantity
    ...
    function _addReserveSnapshot(uint256 price, bool isLiquidation) internal {
    ...
    function setFundingBufferPeriod(uint _fundingBufferPeriod)
        external
        onlyGovernance
    {
    ...
    function setMaxOracleSpreadRatio(uint _maxOracleSpreadRatio)
        external
        onlyGovernance
    {
    ...
    function setLiquidationParams(
        uint _maxLiquidationRatio,
        uint _maxLiquidationPriceSpread
    ) external onlyGovernance {
        maxLiquidationRatio = _maxLiquidationRatio;
        maxLiquidationPriceSpread = _maxLiquidationPriceSpread;
    }

    function setMinSizeRequirement(uint _minSizeRequirement)
        external
        onlyGovernance
    {
        minSizeRequirement = _minSizeRequirement;
