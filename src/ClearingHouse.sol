...
contract ClearingHouse is IClearingHouse, HubbleBase {
    ...
    int256 public override maintenanceMargin;
    uint public override tradeFee;
    uint public override liquidationPenalty;
    ...
    function closePosition(uint idx, uint quoteAssetLimit)
        external
        override
        whenNotPaused
    {
    ...
    function _openPosition(
        ...
        (int realizedPnl, uint quoteAsset, bool isPositionIncreased) = amms[idx]
            .openPosition(trader, baseAssetQuantity, quoteAssetLimit);
        uint _tradeFee = _chargeFeeAndRealizePnL(
            trader,
            realizedPnl,
            quoteAsset,
            false /* isLiquidation */
    ...
    function commitLiquidity(uint idx, uint quoteAsset) external override {
    ...
    function addLiquidity(
        ...
        uint minDToken
    ) external override whenNotPaused returns (uint dToken) {
        address maker = _msgSender();
        updatePositions(maker);
    ...
    function removeLiquidity(
        ...
        address maker = _msgSender();
        updatePositions(maker);
        (int256 realizedPnl, uint quoteAsset, int baseAssetQuantity) = amms[idx]
            .removeLiquidity(maker, dToken, minQuoteValue, minBaseValue);
        marginAccount.realizePnL(maker, realizedPnl);
        if (baseAssetQuantity != 0) {
            emit PositionTranslated(
                maker,
                idx,
                baseAssetQuantity,
                quoteAsset,
    ...
    function updatePositions(address trader) public override whenNotPaused {
    ...
    function settleFunding() external override whenNotPaused {
        uint numAmms = amms.length;
        for (uint i; i < numAmms; ++i) {
    ...
    function liquidate(address trader) external override whenNotPaused {
    ...
    function liquidateMaker(address maker) public override whenNotPaused {
        updatePositions(maker);
        _liquidateMaker(maker);
    }

    function liquidateTaker(address trader) public override whenNotPaused {
    ...
    function _liquidateMaker(address maker) internal {
        ...
        for (uint i; i < l; ++i) {
            ...
            (_realizedPnl, quoteAsset, baseAssetQuantity) = amms[i]
                .forceRemoveLiquidity(maker);
            _isMaker = true;
    ...
    function _liquidateTaker(address trader) internal {
        ...
        for (uint i; i < numAmms; ++i) {
            ...
            if (size != 0) {
                (int _realizedPnl, int _baseAsset, uint _quoteAsset) = _amm
                    .liquidatePosition(trader);
                realizedPnl += _realizedPnl;
    ...
    function _disperseLiquidationFee(uint liquidationFee) internal {
        if (liquidationFee > 0) {
            ...
            marginAccount.transferOutVusd(
                _msgSender(),
                liquidationFee - toInsurance
            );
        }
    }
    ...
    function calcMarginFraction(
        address trader,
        bool includeFundingPayments,
        Mode mode
    ) public view returns (int256) {
        (
            uint256 notionalPosition,
            int256 margin
        ) = getNotionalPositionAndMargin(trader, includeFundingPayments, mode);
        return _getMarginFraction(margin, notionalPosition);
    }

    function isMaker(address trader) public view override returns (bool) {
        uint numAmms = amms.length;
        for (uint i; i < numAmms; ++i) {
    ...
    function getTotalFunding(address trader)
        public
        view
        override
        returns (int256 totalFunding)
    {
        ...
        uint numAmms = amms.length;
        for (uint i; i < numAmms; ++i) {
            (takerFundingPayment, makerFundingPayment, , ) = amms[i]
    ...
    function getTotalNotionalPositionAndUnrealizedPnl(
        address trader,
        int256 margin,
        Mode mode
    )
        ...
        returns (uint256 notionalPosition, int256 unrealizedPnl)
    {
        ...
        for (uint i; i < numAmms; ++i) {
            if (amms[i].isOverSpreadLimit()) {
                (_notionalPosition, _unrealizedPnl) = amms[i].getOracleBasedPnl(
                    trader,
                    margin,
                    mode
                );
            } else {
                (_notionalPosition, _unrealizedPnl, , ) = amms[i]
                    .getNotionalPositionAndUnrealizedPnl(trader);
            }
    ...
    function getNotionalPositionAndMargin(
        address trader,
        bool includeFundingPayments,
        Mode mode
    ) public view override returns (uint256 notionalPosition, int256 margin) {
        ...
        (
            notionalPosition,
            unrealizedPnl
        ) = getTotalNotionalPositionAndUnrealizedPnl(trader, margin, mode);
        margin += unrealizedPnl;
    }

    function getAmmsLength() public view override returns (uint) {
        return amms.length;
    }
    ...
    function isAboveMaintenanceMargin(address trader)
        external
        view
        override
        returns (bool)
    {
        return
            calcMarginFraction(trader, true, Mode.Maintenance_Margin) >=
            maintenanceMargin;
    }
     ...
     */
    function getMarginFraction(address trader)
        external
        view
        override
        returns (int256)
    {
        return
            calcMarginFraction(
                trader,
                true, /* includeFundingPayments */
                Mode.Min_Allowable_Margin
            );
    }
    ...
    function assertMarginRequirement(address trader) public view {
        require(
            calcMarginFraction(trader, false, Mode.Min_Allowable_Margin) >=
                minAllowableMargin,
    ...
    function _assertLiquidationRequirement(address trader) internal view {
        require(
            calcMarginFraction(trader, false, Mode.Maintenance_Margin) <
                maintenanceMargin,
    ...
    function _calculateLiquidationPenalty(uint quoteAsset)
        internal
        view
        returns (uint)
    {
    ...
    function _getMarginFraction(int256 accountValue, uint notionalPosition)
        private
        pure
        returns (int256)
    {
        ...
        return
            (accountValue * PRECISION.toInt256()) / notionalPosition.toInt256();
    }
